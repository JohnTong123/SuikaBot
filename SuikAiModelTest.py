...
class QTrainer:
    ...
    def train_step(self, memory):
        ...
        state_action_values = self.model(state).gather(1, action).max(1).values
        ...
        #                                     next_state)), dtype=torch.bool)
        # non_final_next_states = torch.cat([s for s in next_state
        #                                             if s is not None])
        next_state_values = torch.zeros(BATCH_SIZE)
        with torch.no_grad():
            next_state_values = self.target(next_state).max(1).values
        ...
        temp = expected_state_action_values.unsqueeze(1)
        temp2 = state_action_values.unsqueeze(1)
        loss = self.criterion(temp2, temp)
